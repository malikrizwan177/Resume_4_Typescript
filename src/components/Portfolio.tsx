...
const Portfolio: React.FC = () => {
  ...
  const projects: Project[] = useMemo(
    () => [
      ...
      },
      {
        ...
        description:
          "An AI Themed Frontend website made from a Youtube tutorial with React and Tailwind CSS",
      ...
      },
      {
        ...
        description:
          "A resume made from a Youtube tutorial using React, Threejs, Framer Motion, and Tailwind CSS",
  ...
  const nextSlide = useCallback((): void => {
    setSlide((prev) => (prev === projects.length - 1 ? 0 : prev + 1));
  }, [projects.length]);

  const prevSlide = useCallback((): void => {
    setSlide((prev) => (prev === 0 ? projects.length - 1 : prev - 1));
  }, [projects.length]);

  return (
    <section
      id="projects"
      className="px-8 lg:px-32 py-10 md:py-20 bg-primary-dark"
    >
                ...
                <div key={index}>
                  ...
                  >
                    <img
                      ...
                      } w-[580px] h-[396px] hover:border-2 rounded-xl hover:border-secondary object-contain`}
                    />
                  </a>
                  <div className="flex flex-col justify-center items-center">
                    ...
                    <div>
                      <a
                        href={items.gitLink}
                        className={`${
                          items.gitLink === "blucomm"
                            ? "text-red-500 pointer-events-none"
                            : "text-white"
                        } flex-row justify-center items-center gap-2 rounded-full px-8 py-2 mb-10 mt-5 background-gradient ${
                          slide === index ? "flex" : "hidden"
                        }`}
                      >
                        {items.gitLink === "blucomm"
